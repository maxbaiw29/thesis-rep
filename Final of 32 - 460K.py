...
shortcut = x
x = layers.Conv2D(filters=94, kernel_size=3, strides=1, padding='same')(x)
x = layers.BatchNormalization()(x)
x = layers.Conv2D(filters=94, kernel_size=3, strides=1, padding='same')(x)
x = layers.BatchNormalization()(x)
x = layers.Conv2D(filters=94, kernel_size=3, strides=1, padding='same')(x)
x = layers.BatchNormalization()(x)
shortcut = layers.Conv2D(filters=94, kernel_size=3, strides=1, padding='same')(shortcut)# Adjusted shortcut
x = layers.add([x, shortcut])
x = layers.Activation(activation)(x)
